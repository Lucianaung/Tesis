...
#define MAX_LEDS 497
#define SIZE_VIAJERO 10
#define MAX_VIAJEROS 200

#define MAX_ASISTENTES 200
#define SIZE_ASISTENTE 4
#define SIZE_VIAJERO_ASISTENTE 4
...
CHSV coloresPredefinidos[12] = {
  ...
  CHSV(227, 60, 50),  // rosablanco 11 /120
};
...
// --------------------------------------------
// NUEVA ANIMACIÓN DEL LIENZO BLANCO (versión sin delay, tipo respiración orgánica)
// ---------------------------------------------
void animarLienzoBlanco() {
  ...
  const unsigned long intervalo = 16; // ~60 FPS

  // Inicializar las fases y velocidades solo una vez
  if (!inicializado) {
  ...
  if (ahora - ultimoUpdate < intervalo) return; // control de tiempo sin delay
  ultimoUpdate = ahora;

  // Tomamos el colorFondo (definido en lienzoBlanco) como base
  CHSV fondoHSV = rgb2hsv_approximate(colorFondo);

  uint8_t hueBase = fondoHSV.hue;
  uint8_t satBase = fondoHSV.sat;
  uint8_t brilloBase = fondoHSV.v;

  uint8_t amplitud = 70;   // variación de brillo
  uint8_t brilloMin = brilloBase > amplitud ? brilloBase - amplitud / 2 : brilloBase / 2;
  uint8_t brilloMax = min((int)brilloBase + amplitud, 255);

  for (int i = 0; i < NUM_LEDS; i++) {
    fases[i] += velocidades[i];
    if (fases[i] > 6.2831) fases[i] -= 6.2831;

    float onda = (sin(fases[i]) + 1.0) * 0.5;
    uint8_t brillo = brilloMin + onda * (brilloMax - brilloMin);

    leds[i] = CHSV(hueBase, satBase, brillo);
  }
